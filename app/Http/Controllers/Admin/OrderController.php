...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalOrders = Order::count(); // Ini akan menghitung jumlah semua pesanan

		return view('admin.orders.index', compact('orders','statuses','totalOrders'));
    }
    ...
    {
        // Pastikan hanya pesanan yang belum dibatalkan bisa diproses
        if ($order->status === Order::CANCELLED) {
            return redirect()->back()->with('error', 'Pesanan sudah dibatalkan sebelumnya.');
        }

        $cancelOrder = \DB::transaction(function () use ($order, $request) {
            $params = [
                'status' => Order::CANCELLED,
                'cancelled_by' => auth()->id(), // Admin atau null jika pembeli
                'cancelled_at' => now(),
                'cancellation_note' => $request->input('cancellation_note', 'Pesanan dibatalkan oleh pembeli.'), // Default untuk pembeli
            ];
        ...
        $message = auth()->check() ? 'Pesanan berhasil dibatalkan.' : 'Pesanan Anda berhasil dibatalkan.';
        return redirect('admin/orders')->with('success', $message);
    }
