...
class OrderController extends Controller
{
    ...
    public function checkout()
    {
		...
		return view('frontend.orders.checkout', compact('items','categories','totalWeight','provinces','cities'));
	}
	...
	public function received($orderId)
	{
		...
		return view('frontend.orders.received', compact('order', 'categories'));
	}
	    ...
	    public function markAsReceived($id)
    {
        ...
        // Periksa jika status memenuhi syarat
        if (in_array($order->status, ['paid', 'confirmed', 'delivered', 'completed'])) {
            $order->status = 'completed'; // Tandai sebagai selesai
            $order->save();

            return redirect()->back()->with('success', 'Pesanan telah ditandai sebagai selesai.');
        }

        return redirect()->back()->with('error', 'Status pesanan tidak memenuhi syarat.');
    }
    
        public function doCancel(Request $request, Order $order)
    {
        // Pastikan pengguna hanya bisa membatalkan pesanan miliknya
        if ($order->user_id !== auth()->id()) {
            abort(403, 'Anda tidak memiliki izin untuk membatalkan pesanan ini.');
        }

        // Pastikan hanya pesanan tertentu yang bisa dibatalkan
        if ($order->status === Order::CANCELLED) {
            return redirect()->back()->with('error', 'Pesanan ini sudah dibatalkan sebelumnya.');
        }

        if (in_array($order->status, [Order::COMPLETED, Order::DELIVERED, Order::PAID])) {
            return redirect()->back()->with('error', 'Pesanan ini tidak dapat dibatalkan karena sudah dikirm, selesai atau dibayar.');
        }
        ...
        $order->update([
            ...
            'cancelled_at' => now(),
            'cancellation_note' => $request->input('cancellation_note', 'Pesanan dibatalkan oleh pembeli.'),
        ]);


        return redirect()->route('frontend.orders.show', ['orderId' => $order->id])
