...
class CartController extends Controller
{
    public function index()
    {
        ...
        return view('frontend.carts.index', compact('items', 'categories'));
    }
    ...
    public function shopNow(Request $request)
    {
        ...
        $attributes = [];
        if ($product->configurable()) {
            // Logika untuk produk yang bisa dikonfigurasi
            $product = Product::from('products as p')
                ->whereRaw(
                    "p.parent_id = :parent_product_id
                and (select pav.text_value
                ...
                and (select pav.text_value
                    ...
                    [
                        'parent_product_id' => $product->id,
                        'size_code' => 'size',
                        'size_value' => $params['size'] ?? null,
                        'color_code' => 'color',
                        'color_value' => $params['color'] ?? null,
                    ]
                )->firstOrFail();

            $attributes['size'] = $params['size'] ?? null;
            $attributes['color'] = $params['color'] ?? null;
        }
