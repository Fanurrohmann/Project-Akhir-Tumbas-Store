        ...
        <div class="modal-body">
            <div class="qwick-view-left">
                    ...
                    <div class="quick-view-tab-content tab-content">
                        ...
                        @foreach ($product->productImages as $image)
                            <div class="tab-pane fade {{ $i == 1 ? 'active show' : '' }}"
                                id="modal{{ $i }}" role="tabpanel">
                                @if ($image->path)
                                    <img src="{{ asset('storage/' . $image->path) }}" alt="{{ $product->name }}"
                                        style="width:320px;">
                                @else
                                    <img src="{{ asset('themes/TumbasStore/assets/img/quick-view/l3.jpg') }}" alt="">
                                @endif
                            </div>
                            @php
                                $i++;
                ...
                <div class="quick-view-list nav" role="tablist">
                    ...
                    @endphp
                    @foreach ($product->productImages as $image)
                        <a class="{{ $i == 1 ? 'active' : '' }} mr-12" href="#modal{{ $i }}"
                            data-toggle="tab" role="tab">
                            @if ($image->path)
                                <img src="{{ asset('storage/' . $image->path) }}" alt="{{ $product->name }}"
                                    style="width:100px; height:112px">
                            @else
                                <img src="{{ asset('themes/TumbasStore/assets/img/quick-view/s3.jpg') }}"
                                    alt="{{ $product->name }}">
                            @endif
                        </a>
                        @php
            ...
            <div class="qwick-view-right">
                <div class="qwick-view-content">
                    <h3>{{ $product->name }}</h3>
                    <div class="price">
                        <span class="new">Rp. {{ number_format($product->priceLabel(), 0, ',', '.') }}</span>
